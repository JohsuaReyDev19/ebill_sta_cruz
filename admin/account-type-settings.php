...
<body id="page-top">
    <div class="d-none" id="account-type-settings"></div>
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                            ...
                            <div class="card shadow mb-4">
                                ...
                                <div class="card-header py-3 d-flex flex-column flex-md-row">
                                    <div class="col-12 col-md-6 d-flex align-items-center justify-content-start mx-0 px-0 mb-2 mb-md-0">
                                        <h6 class="font-weight-bold text-primary mb-0">List of Account Type</h6>
                                    </div>
                                    <div class="col-12 col-md-6 d-flex align-items-center justify-content-end mx-0 px-0">
                                        <div class="col-12 col-md-4 float-right mx-0 px-0">
                                            <a data-toggle="modal" data-target="#addNew" class="btn btn-success shadow-sm w-100 h-100"><i class="fa-solid fa-plus mr-1"></i>Add Account Type</a>
                                        </div>
                                    </div>
                                </div>
                                <!-- Card Body -->
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered nowrap" id="myTable" width="100%" cellspacing="0">
                                            ...
                                            <?php
                                                ...
                                                $display_account_type = "SELECT * FROM account_type_settings WHERE deleted = 0";
                                                $sqlQuery = mysqli_query($con, $display_account_type) or die(mysqli_error($con));
                                                ...
                                                while($row = mysqli_fetch_array($sqlQuery)) {
                                                    ...
                                                    $account_type_remarks = $row['account_type_remarks'];
                                                    
                                            ?>

                                                <tr>
                                                    <td><?php echo $counter; ?></td>
                                                    <td><?php echo htmlspecialchars($account_type); ?></td>
                                                    <td><?php echo htmlspecialchars($account_type_remarks); ?></td>
                                                    <td class="text-center">
                                                        <a class="btn btn-sm btn-primary" data-toggle="modal" data-target="#edit_<?php echo $account_type_id; ?>"><i class="fa-solid fa-edit"></i></a>
                                                        <a href="#" class="btn btn-sm btn-danger delete-accounttype-btn"
                                                           data-accounttype-id="<?php echo $account_type_id; ?>"
                                                           data-accounttype-name="<?php echo htmlspecialchars($account_type); ?>"
                                                           data-accounttype-remarks="<?php echo htmlspecialchars($account_type_remarks); ?>">
                                                ...
                                                <?php
                                                    $counter++;
                                                   include('modal/account_type_edit_modal.php');
                                                }
                                                ?>
                                            </tbody>
                                        </table>
                        ...
                        <?php include('modal/account_type_add_modal.php'); ?>
                    </div>
...
$(document).ready(function () {
    ...
    $('#addAccountType').on('click', function (e) {
        e.preventDefault();
        ...
        // AJAX SUBMISSION
        $.ajax({
            url: 'action/add_account_type.php',
            type: 'POST',
            data: form.serialize(),
            success: function (response) {
                ...
                if (response === 'success') {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Failed to add Account Type',
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.delete-accounttype-btn').on('click', function(e) {
                e.preventDefault();

                var deleteButton = $(this);
                var classificationID = deleteButton.data('accounttype-id');
                var classificationName = decodeURIComponent(deleteButton.data('accounttype-name'));
                var classificationRemarks = decodeURIComponent(deleteButton.data('accounttype-remarks'));

                Swal.fire({
                    title: 'Delete Account Type',
                    html: "You are about to delete the following Account Type:<br><br>" +
                          "<strong>Account Type:</strong> " + classificationName + "<br>" +
                          "<strong>Account Type Remarks:</strong> " + classificationRemarks + "<br>",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: 'action/delete_account_type.php',
                            type: 'POST',
                            data: {
                                account_type_id: classificationID
                            },
                            success: function(response) {
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '[id^="updateAccountType_"]', function(e) {
                ...
                if (fieldsAreValid) {
                    $.ajax({
                        url: 'action/update_account_type.php', // URL to submit the form data
                        type: 'POST',
